...
def _apply_cli_overrides(
    ...
    Args:
        ...
        workers: Requested worker count, or None to auto-detect.

    Returns:
        The settled modes, so a failure *after* this point can report
        what the run had established rather than claiming it never got
        that far.
    """
    ...
    # Settle and validate the modes before anything repository-dependent.
    # An unsupported rung is a usage error, and a usage error must not
    # arrive after a cooldown notice -- or after a malformed Dependabot
    # file has already failed the run for an unrelated reason.
    _apply_check_modes(config, options)

    # Reported by a failure later in setup -- the cooldown lookup can
    # raise on an unreadable Dependabot file -- so such a document
    # states the modes the run had settled rather than claiming it never
    # reached them. Returned here, before anything that can raise, for
    # exactly that reason.
    return _check_modes(config, action_calls_ran=False, allow_list_ran=False)


def _resolve_cooldown(config: Config, options: CLIOptions) -> None:
    """Settle the action-update cooldown window.

    Precedence: the explicit ``--cooldown`` flag, then the repository's
    Dependabot configuration, then 0 (no cooldown / original
    behaviour).

    Kept out of :func:`_apply_cli_overrides` because it reads a file and
    can therefore fail. Everything that settles configuration from
    arguments alone happens first and is reportable; this is the first
    step that depends on the repository.

    A sweep skips it. The cooldown belongs to each checkout, and
...
def _unsettled_check_modes() -> dict[str, dict[str, Any]]:
    """Describe every check for a run that never settled its modes.

    A setup failure is refused before the modes resolve -- and may be
    refused *by* resolving them, as an unsupported rung is. So the modes
    are genuinely unknown, and ``None`` says so rather than guessing a
    default the caller may not have asked for.

    The entries themselves are not optional. ``checks`` is documented as
    always carrying one entry per check, and a consumer that has to
    handle an empty mapping on some paths gains nothing from the key on
    the others.

    Returns:
        One entry per check, each unrun and with no known mode.
    """
...
def _emit_setup_failure(
    ...
    *,
    ...
    Args:
        ...
        check_modes: The modes, when the run settled them before
...
def _emit_results(
    ...
    *,
    ...
    # What actually happened, decided once and used for the document,
    # the counts and the notice alike, so the three cannot disagree.
    action_calls_ran = _stage_ran(config.action_calls_mode, rate_limited)
    # Not derived from the mode: _run_allow_list_stage returns None both
    # when the check is off and when an advisory failure made it skip,
    # and a check that failed to run must not read as one that passed.
    allow_list_ran = allow_list is not None and allow_list.checked
    ...
    # Validation counts describe validation, so a stage that never ran
    # contributes none. The scan summary keeps its real figures: the
    # scan did happen, and saying "23 calls found, 0 validated" is the
    # accurate pair. Reporting 23 validated beside "checking is off"
    # was not.
    validation_summary = validation.validator.get_validation_summary(
        validation.validation_errors,
        validation.total_calls if action_calls_ran else 0,
    ...
    output_text_results(
        ...
        rate_limited=rate_limited,
        action_calls_ran=action_calls_ran,
        action_calls_off=not config.action_calls_mode.runs,
    )
...
def output_text_results(
    ...
    *,
    ...
    if not quiet:
        ...
        if action_calls_off:
            # Deliberately keyed on the *mode*, not on whether the stage
            # ran. A throttled run also fails to check anything, but it
            # was not switched off, and the rate-limit notice below says
            # what actually happened. Reporting a throttle as a user
            # decision sends the reader to their own command line to
            # find a flag they never wrote.
            console.print(
        ...
        if rate_limited:
            # Generic about *which* check: with '--action-calls off' and
            # an enabled allow-list, pre-flight still runs and can
            # throttle, so naming action calls here would blame a check
            # the caller disabled and say nothing about the one that was
            # actually skipped.
            #
            # Silence is the alternative, and worse: "All action calls
            # are valid" would be the text-mode twin of the
            # byte-identical JSON document -- a run that checked nothing,
            # reporting no problems, read as a clean result.
            console.print(
                "[yellow]GitHub API rate-limited; enabled checks did not "
                "run \u26a0\ufe0f[/yellow]"
            )
    ...
    *,
    ...
    Args:
        ...
        check_modes: Each check's mode and whether it executed, by check
            ...
            the text notice. Omitted only by a run that failed before
            settling its modes, which reports every check as unrun with
            no known mode rather than reporting nothing.
    ...
    result = {
        ...
        "checks": dict(
            sorted((check_modes or _unsettled_check_modes()).items())
        ),
