...
class ValidationCache:
    ...
    def _load_cache(self) -> None:
        ...
        try:
            ...
            for repo, version_info in latest_versions_data.items():
                try:
                    self._latest_versions[repo] = LatestVersionEntry(**version_info)
                except Exception as e:
                    self.logger.debug(f"Failed to load latest version for {repo}: {e}")
            ...
            if cache_version != __version__:
                if cache_version:
                    ...
                    Console().print("[cyan]♻️  Cache version mismatch; purging cache to ensure consistency[/cyan]")
                else:
                    self.logger.debug(
    ...
    def _cleanup_expired(self) -> None:
        ...
        for repo, entry in self._latest_versions.items():  # type: ignore[assignment]
            if entry.is_expired(self.config.default_ttl_seconds):
        ...
        if expired_version_repos:
            self.logger.debug(f"Removed {len(expired_version_repos)} expired latest version entries")
    ...
    def get_cache_info(self) -> dict[str, Any]:
        ...
        for entry in self._latest_versions.values():  # type: ignore[assignment]
            if entry.is_expired(self.config.default_ttl_seconds):
    ...
    def get_latest_version(self, repository: str) -> tuple[str, str] | None:
        ...
        if version_entry:
            ...
            if version_entry.is_expired(self.config.default_ttl_seconds):
                self.logger.debug(f"Latest version cache for {repository} has expired")
                del self._latest_versions[repository]
                return None

            self.logger.debug(f"Cache hit for latest version: {repository} -> {version_entry.latest_tag}")
            return (version_entry.latest_tag, version_entry.latest_sha)

        return None

    def put_latest_version(self, repository: str, latest_tag: str, latest_sha: str) -> None:
        """
        ...
        self.logger.debug(f"Cached latest version for {repository}: {latest_tag} ({latest_sha[:8]}...)")
