...
class AutoFixer:
    ...
    def __init__(self, config: Config, base_path: Path | None = None) -> None:
        ...
        self._latest_versions_cache: dict[str, tuple[str, str, float]] = {}  # {repo: (tag, sha, timestamp)}
        self._cache_ttl = 300  # 5 minutes

        # Redirect tracking
        self._redirects_seen: set[str] = set()  # Track redirects we've already displayed
        self._redirects_found: set[str] = set()  # Track unique redirected actions
        self._redirect_updates: int = 0  # Count of action calls updated due to redirects
    ...
    async def __aenter__(self) -> AutoFixer:
        ...
        if self.config.effective_github_token:
            headers["Authorization"] = f"token {self.config.effective_github_token}"
    ...
    async def fix_validation_errors(
        ...
        all_action_calls: dict[Path, dict[int, ActionCall]] | None = None,
        dry_run: bool = False,
    ) -> tuple[dict[Path, list[dict[str, str]]], dict[str, int], dict[str, list[dict[str, Any]]]]:
        """
        Fix validation errors in workflow files.

        Args:
            errors: List of validation errors to fix
            all_action_calls: Optional dict of all action calls for version updates when auto_latest is enabled
            dry_run: If True, check for updates but don't apply them (report mode)
        ...
        Returns:
            ...
            - Dictionary mapping relative file paths to lists of stale action info (for dry_run mode)
        """
        if not self.config.auto_fix:
            ...
            if not self.config.fix_test_calls:
                return self._collect_skipped_testing_items(errors), {"actions_moved": 0, "calls_updated": 0}, {}
        ...
        # Create a set of validation error action calls that need fixing regardless of dry_run mode
        ...
        for error in errors:
            ...
            if error.result == ValidationResult.INVALID_REFERENCE:
                ...
                should_fix = False
            elif error.result in [ValidationResult.INVALID_SYNTAX, ValidationResult.NETWORK_ERROR, ValidationResult.TIMEOUT]:
            ...
            if should_fix:
                ...
                # Track this as a validation error that should be fixed regardless of dry_run
                validation_error_calls.add((error.file_path, error.action_call.line_number))

        # If all_action_calls is provided, use batch processing for all fixes
        # Pass validation_error_calls so they get fixed even in dry_run mode
        # In dry_run mode, version updates are only reported, but validation errors are still fixed
        if all_action_calls:
            console = Console()
            mode_msg = "dry-run mode" if dry_run else "auto_latest enabled"
            self.logger.debug(f"Checking all action calls for latest versions ({mode_msg})")

            # Use batch processing for performance
            with Live("", console=console, refresh_per_second=4) as live:
                version_fixes, version_skipped, stale_summary = await self._process_action_calls_batch(
                    all_action_calls, live, dry_run=dry_run, validation_error_calls=validation_error_calls
                )
                # In dry_run mode, version_fixes will be empty (only stale_summary is populated)
                # In normal mode, version_fixes contains the actual version updates to apply
            ...
            # If all_action_calls is not provided, fix validation errors individually
            for error in errors:
                # Only process if this was marked as a fixable validation error
                if (error.file_path, error.action_call.line_number) in validation_error_calls:
                    try:
                        result = await self._fix_action_call_with_redirect(error.action_call, error.result, live=None)
                        fixed_line = result.get("fixed_line") if result else None
                        if fixed_line and fixed_line != error.action_call.raw_line.strip():
                            ...
                            file_name = error.file_path.name
                            self.logger.debug(
                                f"Fixed {error.action_call.organization}/{error.action_call.repository} in {file_name} (line {error.action_call.line_number})"
                            )

                            # Track if this update was due to a redirect
                            if result and result.get("redirect_info"):
    ...
    async def _process_action_calls_batch(
    ...
    ) -> tuple[dict[Path, dict[int, tuple[str, str]]], dict[Path, dict[int, str]], dict[str, list[dict[str, Any]]]]:
        """
        Process all action calls in batch for auto_latest mode.
        ...
        3. Fetches all SHAs in parallel/batch
        4. Applies fixes using pre-fetched data
        ...
        Args:
            ...
            validation_error_calls: Set of (file_path, line_number) tuples for validation errors
                                   that should be fixed even in dry_run mode

        Returns:
            Tuple of (fixes_by_file, skipped_by_file, stale_actions_summary)
        """
        fixes_by_file: dict[Path, dict[int, tuple[str, str]]] = {}
        skipped_by_file: dict[Path, dict[int, str]] = {}
        stale_actions_summary: dict[str, list[dict[str, Any]]] = defaultdict(list)
        ...
        for file_path, calls in all_action_calls.items():
            for line_num, action_call in calls.items():
                ...
                if not self.config.fix_test_calls and has_test_comment(action_call):
                    if file_path not in skipped_by_file:
                        skipped_by_file[file_path] = {}
                    skipped_by_file[file_path][line_num] = action_call.raw_line.strip()
                    continue
        ...
        live.update(Text(f"  Fetching latest versions for {len(unique_repos)} unique repositories...", style="dim"))

        # Step 2: Batch-fetch latest versions for all unique repos
        latest_versions = await self._get_latest_versions_batch(list(unique_repos))
        ...
        # Step 3b: Fetch latest versions for redirected repositories
        if redirected_repos:
            live.update(Text(f"  Fetching latest versions for {len(redirected_repos)} redirected repositories...", style="dim"))
            redirected_versions = await self._get_latest_versions_batch(redirected_repos)
            latest_versions.update(redirected_versions)

        # Step 3c: Collect refs that need SHA resolution
        for repo_key in unique_repos:
            ...
            if effective_repo in latest_versions:
                ...
                if not sha:  # SHA not available yet, need to resolve
                    refs_to_resolve.append((effective_repo, tag))

        # Step 4: Batch-fetch SHAs for all refs that need resolution
        if refs_to_resolve:
            live.update(Text(f"  Resolving SHAs for {len(refs_to_resolve)} references...", style="dim"))
            sha_map = await self._get_shas_batch(refs_to_resolve)
        else:
            sha_map = {}

        # Step 5: Check for updates using pre-fetched data
        live.update(Text(f"  Checking {len(action_call_list)} action calls for updates...", style="dim"))

        for file_path, line_num, action_call in action_call_list:
            try:
                repo_key = f"{action_call.organization}/{action_call.repository}"
                base_repo_key = self._get_base_repository(repo_key)
                original_base_repo = base_repo_key  # Keep original for comparison

                # Get relative path for reporting
                relative_path = str(file_path.relative_to(self.base_path)) if self.base_path else str(file_path)
                ...
                effective_lookup_repo = base_repo_key  # Use the potentially redirected repo
                if effective_lookup_repo in latest_versions:
                    ...
                    if cached_sha:
                        ...
                        target_sha = sha_map[(effective_lookup_repo, target_ref)]
                    else:
                        # Fallback to individual fetch (shouldn't happen often)
                        sha_info = await self._get_commit_sha_for_reference(effective_lookup_repo, target_ref)
                        target_sha = sha_info["sha"] if sha_info else None

                    if target_sha:
                        # Check if this is actually a change
                        final_ref = target_sha if self.config.require_pinned_sha else target_ref
                        version_comment: str | None = target_ref if re.match(r"^v?\d+\.\d+", target_ref) else None
                        ...
                        repo_changed = repo_was_redirected or (base_repo_key != original_base_repo)
                        comment_changed = (version_comment != existing_comment) if version_comment else False
                        ...
                        if ref_changed or comment_changed or repo_changed:
                            ...
                            if fixed_line and fixed_line != action_call.raw_line.strip():
                                # Increment redirect counter if this change is due to a redirect
                                if repo_was_redirected:
                                    self._redirect_updates += 1

                                # Check if this is a validation error that should always be fixed
                                is_validation_error = validation_error_calls and (file_path, line_num) in validation_error_calls

                                if dry_run and not is_validation_error:
                                    # In dry-run mode, collect stale action info for reporting (but not validation errors)
                                    stale_actions_summary[relative_path].append({
                                        "line": line_num,
                                        "action": repo_key if repo_was_redirected else f"{action_call.organization}/{action_call.repository}",
                                    ...
                                    # In normal mode OR validation error in dry-run mode, apply the fix
                                    if file_path not in fixes_by_file:
                                        fixes_by_file[file_path] = {}
        ...
        return fixes_by_file, skipped_by_file, dict(stale_actions_summary)
    ...
    async def _fix_action_call_with_redirect(
        ...
        if new_base_repo:
            ...
            if len(repo_key.split("/")) > 2:
                ...
                repo_key = new_base_repo
            base_repo_key = new_base_repo
            self.logger.debug(f"Using redirected repository: {action_call.organization}/{action_call.repository} -> {repo_key}")
        ...
        # Try to fix the current reference based on validation error type
            ...
            target_ref = await self._find_valid_reference(base_repo_key, action_call.reference)
            if not target_ref:
                target_ref = await self._get_fallback_reference(base_repo_key, action_call.reference)
        ...
        if self.config.require_pinned_sha or action_call.reference_type != ReferenceType.COMMIT_SHA:
            ...
            sha_info = await self._get_commit_sha_for_reference(base_repo_key, target_ref)
            if sha_info:
            ...
            # Without access to resolve SHAs, we can't fix NOT_PINNED_TO_SHA issues
                ...
                self.logger.debug(f"Cannot resolve SHA for {base_repo_key}@{target_ref}, skipping SHA pinning")
                return {"fixed_line": None, "redirect_info": redirect_info} if redirect_info else None
            ...
            if not target_sha and target_ref and re.match(r"^v?\d+\.\d+", target_ref):
                version_comment = target_ref

        # Check if we actually have a change to make
        final_ref = target_sha or target_ref
        repo_changed = (base_repo_key != self._get_base_repository(f"{action_call.organization}/{action_call.repository}"))
        ...
        if final_ref == action_call.reference and not comment_changed and not repo_changed:
            # No actual change needed
            return {"fixed_line": None, "redirect_info": redirect_info} if redirect_info else None

        # Build the fixed line - pass new repo if it changed
        fixed_line = self._build_fixed_line(action_call, final_ref, version_comment, repo_key if repo_changed else None)
    ...
    async def _get_commit_sha_for_reference(self, repo_key: str, ref: str) -> dict[str, Any] | None:
        ...
        if self.config.validation_method == ValidationMethod.GIT and self._git_client:
            try:
                ...
                cmd = ["git", "ls-remote", "--tags", url, f"refs/tags/{ref}", f"refs/tags/{ref}^{{}}"]
                try:
                    result = subprocess.run(
                        cmd, capture_output=True, text=True,
                        timeout=self.config.git.timeout_seconds, check=True
    ...
    async def _get_latest_versions_batch(self, repo_keys: list[str]) -> dict[str, tuple[str, str]]:
        """
        Batch-fetch latest versions for multiple repositories.

        Returns dict mapping repo_key to (tag, sha) tuple.
        Uses session cache and supports both GraphQL and Git operations.
        """
        ...
        if self.config.validation_method == ValidationMethod.GITHUB_API and self._graphql_client:
            try:
                graphql_results = await self._get_latest_versions_graphql_batch(repos_to_fetch)
                ...
                repos_to_fetch = [repo for repo in repos_to_fetch if repo not in graphql_results]
                if not repos_to_fetch:
                    return results

                self.logger.debug(f"GraphQL returned results for {len(graphql_results)} repos, falling back to REST API for {len(repos_to_fetch)} repos")
            except Exception as e:
                self.logger.debug(f"GraphQL batch fetch failed, falling back to individual queries: {e}")

        # Fallback to parallel REST API or Git operations
        tasks = [self._get_latest_version_single(repo_key) for repo_key in repos_to_fetch]
        fetch_results = await asyncio.gather(*tasks, return_exceptions=True)

        for repo_key, result in zip(repos_to_fetch, fetch_results):
            if isinstance(result, Exception):
                self.logger.debug(f"Failed to fetch latest version for {repo_key}: {result}")
                continue
    ...
    async def _get_latest_versions_graphql_batch(self, repo_keys: list[str]) -> dict[str, tuple[str, str]]:
        """
        ...
        try:
            if not self._graphql_client:
                return {}
            response_data = await self._graphql_client._execute_graphql_query(query)
            ...
            for alias, repo_key in aliases.items():
                ...
                if not self.config.allow_prerelease and repo_data.get("latestRelease"):
                    ...
                    if ActionCallPatterns.VERSION_TAG_PATTERN.match(tag) or re.match(r"^v?\d+\.\d+", tag):
                        results[repo_key] = (tag, sha)
                        continue
                ...
                if refs:
                    ...
                    for ref in refs:
                        if ActionCallPatterns.VERSION_TAG_PATTERN.match(ref["name"]):
                            ...
                            if "oid" in target:
                                # Direct commit
                                version_tags.append((ref["name"], target["oid"]))
                            elif "target" in target and "oid" in target["target"]:
                                # Annotated tag pointing to commit
                                version_tags.append((ref["name"], target["target"]["oid"]))
                    if version_tags:
                        # Sort by version
                        sorted_tags = sorted(version_tags, key=lambda x: _parse_version(x[0]), reverse=True)
                        ...
                        for ref in refs:
                            if fallback_pattern.match(ref["name"]):
                                ...
                                if "oid" in target:
                                    # Direct commit
                                    fallback_tags.append((ref["name"], target["oid"]))
                                elif "target" in target and "oid" in target["target"]:
                                    # Annotated tag pointing to commit
                                    fallback_tags.append((ref["name"], target["target"]["oid"]))
                        if fallback_tags:
    ...
    async def _get_latest_version_single(self, repo_key: str) -> tuple[str, str] | None:
        """
        ...
        if self.config.validation_method == ValidationMethod.GITHUB_API and self._http_client:
            try:
                # Try latest release
                response = await self._http_client.get(f"https://api.github.com/repos/{repo_key}/releases?per_page=100")
                if response.status_code == 200:
                    ...
                    if self.config.allow_prerelease:
                        clean_releases = [
                            r for r in releases
                            if ActionCallPatterns.VERSION_TAG_PATTERN.match(r.get("tag_name", ""))
                ...
                # Fall back to tags
                response = await self._http_client.get(f"https://api.github.com/repos/{repo_key}/tags?per_page=100")
                if response.status_code == 200:
                    tags = response.json()
        ...
        if self.config.validation_method == ValidationMethod.GIT and self._git_client:
            try:
                ...
                if git_tags:
                    ...
                    if clean_tags:
                        sorted_versions = sorted(clean_tags, key=_parse_version, reverse=True)
    ...
    async def _get_shas_batch(self, refs: list[tuple[str, str]]) -> dict[tuple[str, str], str]:
        """
            ...
            try:
                ...
                for full_alias, (repo_key, ref) in aliases.items():
                    ...
                    if match:
                        ...
                        repo_data = response_data.get("data", {}).get(repo_alias, {})
                        ref_data = repo_data.get(ref_alias)
                        if ref_data and ref_data.get("target"):
                            sha = ref_data["target"]["oid"]
                            results[(repo_key, ref)] = sha
                    else:
                        self.logger.warning(f"Failed to parse alias format: {full_alias}")

                return results
            except Exception as e:
                self.logger.debug(f"GraphQL batch SHA fetch failed, falling back: {e}")

        # Fall back to parallel individual fetches
        tasks = [self._get_commit_sha_for_reference(repo_key, ref) for repo_key, ref in refs]
        fetch_results = await asyncio.gather(*tasks, return_exceptions=True)

        for (repo_key, ref), result in zip(refs, fetch_results):
            if isinstance(result, Exception):
                self.logger.debug(f"Failed to fetch SHA for {repo_key}@{ref}: {result}")
                continue
    ...
    def _build_fixed_line(self, action_call: ActionCall, new_ref: str, version_comment: str | None = None, new_repo: str | None = None) -> str:
