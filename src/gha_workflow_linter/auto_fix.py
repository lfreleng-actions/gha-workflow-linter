...
class AutoFixer:
    ...
    async def fix_validation_errors(
        ...
        Args:
            ...
            all_action_calls: Optional dict of all action calls to check for updates/fixes
        ...
        # If all_action_calls is provided, use batch processing
        ...
        if all_action_calls:
            ...
            if check_for_updates:
                self.logger.debug("Checking all action calls for updates (--auto-latest enabled)")
            else:
                self.logger.debug("Checking all action calls (will fix validation errors and report outdated versions)")
            ...
            with Live("", console=console, refresh_per_second=4) as live:
                version_fixes, version_skipped, stale_summary = await self._process_action_calls_batch(
                    all_action_calls, live, check_for_updates=check_for_updates, validation_error_calls=validation_error_calls
                )
    ...
    async def _process_action_calls_batch(
        ...
        fixes_by_file: dict[Path, dict[int, tuple[str, str]]] = {}
        skipped_by_file: dict[Path, dict[int, str]] = {}
        outdated_actions_summary: dict[str, list[dict[str, Any]]] = defaultdict(list)
        ...
        async def check_redirect_with_limit(repo_key: str) -> tuple[str, str | None]:
            ...
            try:
                ...
                if effective_lookup_repo in latest_versions:
                    ...
                    if target_sha:
                        ...
                        if existing_comment and action_call.reference_type == ReferenceType.COMMIT_SHA:
                            # If the comment looks like a version tag, check if current SHA matches that version
                            if re.match(r"^v?\d+\.\d+", existing_comment):
                                # Fetch the SHA for the existing comment version
                                try:
                                    existing_sha_info = await self._get_commit_sha_for_reference(effective_lookup_repo, existing_comment)
                                    if existing_sha_info and existing_sha_info.get("sha"):
                                        existing_version_sha = existing_sha_info["sha"]
                                        # If current SHA doesn't match what the comment claims, it's corrupted/mismatched
                                        if action_call.reference != existing_version_sha:
                                            has_mismatched_sha = True
                                except Exception:
                                    # If we can't verify, don't mark as mismatched
                                    pass
                        ...
                        if ref_changed or comment_changed or repo_changed:
                            ...
                            if fixed_line and fixed_line != action_call.raw_line.strip():
                                ...
                                is_invalid = is_validation_error or has_mismatched_sha
                                ...
                                if not check_for_updates and not is_invalid:
                                    ...
                                    outdated_actions_summary[relative_path].append({
                                        "line": line_num,
