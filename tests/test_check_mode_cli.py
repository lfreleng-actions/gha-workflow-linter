    ...
    def test_report_does_not_yet_detect_staleness(
        ...
        class RecordingFixer:
            ...
            def __init__(self, *args: object, **kwargs: object) -> None:
                """Accept AutoFixer's construction signature.
            ...
            async def __aenter__(self) -> RecordingFixer:
                """Returns:
                Itself, as the async context manager.
                """
                return self

            async def __aexit__(self, *args: object) -> bool:
                """Args:
                    *args: Exception details, ignored.
            ...
            async def fix_validation_errors(
                ...
                *,
                ...
                """Record the calls offered for examination.

                Args:
                    errors: Validation errors, ignored.
                    all_calls: The calls the fixer may examine.
                    check_for_updates: Whether to look for newer releases.
        ...
        cli._run_auto_fix_stage(
            ...
            cli._ValidationOutcome(
                workflow_calls,
                [],
                ActionCallValidator(config),
                1,
            ),
        )
...
class TestOffMeansTheCheckDoesNotRun:
    ...
    def test_a_setup_failure_still_names_every_check(
        self, temp_dir: Path
    ) -> None:
        """The contract holds on the path that settles no modes at all.

        An unsupported rung is refused while resolving the modes, so the
        document cannot report what they were. It still reports every
        check, unrun, rather than an empty mapping a consumer would have
        to special-case.
    ...
    def test_a_settled_failure_reports_the_modes_it_settled(
        self, temp_dir: Path
    ) -> None:
        """``null`` must mean "never established", not "not recorded".

        Setup does not fail in one place. Resolving the modes can
        refuse an unsupported rung, and the cooldown lookup *after* it
        reads a file that may be unreadable. Both reach the same
        handler, and only the first is entitled to report no modes.

        Args:
            temp_dir: Scratch directory carrying an unreadable
                Dependabot file, so the cooldown lookup fails.
        """
        dependabot = temp_dir / ".github" / "dependabot.yml"
        dependabot.parent.mkdir(parents=True, exist_ok=True)
        dependabot.write_bytes(b"\xff\xfe not utf-8")
        ...
        assert result.exit_code != 0
        assert document["error"]
        # Settled before the failure, so reported.
...
class TestDeprecatedSpellingsStillWork:
    ...
    def test_no_allow_list_is_unaffected(self, temp_dir: Path) -> None:
        ...
        Parsed from ``result.stdout`` rather than ``result.output``.
        The latter is Click's mixed stream, and this invocation emits a
        deprecation notice on stderr. JSON mode happens to suppress it
        today, because ``--format json`` implies ``--quiet`` -- but that
        is a coupling elsewhere in the command, not a property of this
        test, and the document belongs on stdout regardless.
