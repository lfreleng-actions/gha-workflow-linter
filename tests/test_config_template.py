...
class TestGeneratedTemplateCoverage:
    ...
    def test_excluded_fields_are_omitted(
        ...
        Nested blocks are checked too: the rendering is recursive, so a
        top-level-only assertion would miss a nested runtime field
        leaking through.
        """
        excluded = {
        ...
        nested_excluded = 0
        for block, model in NESTED_BLOCKS:
            hidden = {
                name
                for name, field in model.model_fields.items()
                if field.exclude
            }
            nested_excluded += len(hidden)
            rendered: Any = generated_data[block]
            assert not (hidden & set(rendered)), (
                f"{block} exposes runtime-only field(s): {sorted(hidden)}"
            )

        assert nested_excluded, (
            "expected at least one runtime-only field in a nested block"
        )
